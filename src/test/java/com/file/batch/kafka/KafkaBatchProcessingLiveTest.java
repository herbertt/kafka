...
import com.file.batch.kafka.util.BatchConsumer;
import com.file.batch.kafka.util.Producer;
...
@EnableKafka
@EmbeddedKafka(partitions = 1, topics = { "batch_topic" }, brokerProperties = {"listeners=PLAINTEXT://localhost:9092", "port=9092" })
public class KafkaBatchProcessingLiveTest {
    ...
    private void publishMessages() throws ExecutionException, InterruptedException {
        int count = 1;
        String messageTemplate = "KPI Message-";
        while (count <= 100) {
            logger.info("publishing message number {}", count);
            producer.sendMessage("batch_topic", messageTemplate.concat(Integer.valueOf(count).toString()));
            count++;
        }
    }
